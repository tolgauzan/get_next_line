...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/25 01:19:25 by tuzan             #+#    #+#             */
/*   Updated: 2024/11/25 01:19:25 by tuzan            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_update_content(char *content)
{
	char	*new_content;
	int		i;
	int		j;
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || fd >= FD_MAX || BUFFER_SIZE <= 0)
		return (NULL);
	content[fd] = ft_readline(fd, content[fd]);
	if (!content[fd])
		return (NULL);
	line = ft_parseline(content[fd]);
	if (!line)
		return (free(content[fd]), content[fd] = NULL, NULL);
